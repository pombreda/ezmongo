...
class Schema(object):
    ...
    def add_array(self, name, content_type=str, schema=None, always_unwind=False):
        ...
        """
        self.array_fields[name] = Field(name, "array", content_type, schema, always_unwind=always_unwind)


    def add_fields(self, flds, fldtype=str):
        if isinstance(flds, list):
            for fld in flds:
                self.fields[fld] = Field(fld, fldtype)
        else:
            for fld, tmpfldtype in flds.items():
                self.fields[fld] = Field(fld, tmpfldtype)

    def is_array(self, fldname):
        return fldname in self.array_fields

    def get_fld_val(self, fldname, fldval):
        return self.fields.get(fldname, None)


    def get_always_unwind(self, fldname):
        return fldname in self.array_fields and self.array_fields.get(fldname).always_unwind == True

    def __str__(self):
        return """
        Fields: %s
        Arrays: %s
        """ % ([fld.name for fld in self.fields.values()], [[fld.name, fld.type, fld.content_type, str(fld.schema)] for fld in self.array_fields.values()])
